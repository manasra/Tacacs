    ...
    def verify_aaa_group(self, connection, device_type: str) -> TestResult:
        ...
        try:
            if device_type == 'cisco_nxos':
                ...
                # Try multiple commands for IOS
                output = connection.send_command('show running-config | include aaa group server')
                output2 = connection.send_command('show running-config | section aaa group server tacacs')
                output = output + "\n" + output2
            ...
            if 'ISE-GROUP' in output:
                ...
                return TestResult(
                    name="AAA Server Group",
                    passed=False,
                    message="ISE-GROUP not found in running config"
                )
    ...
    def configure_device(self, device: Dict) -> DeviceResult:
        ...
        try:
            ...
            if all_passed:
                ...
                self.logger.warning(f"\n[WARNING] Some tests failed: {', '.join(failed_tests)}")
